...
function loadDB(callback) {
    ...
    if (callback != undefined) {
        ...
        database.on('value', (dbValue) => {   
            getList = [];
            dbValue.forEach((childdbValue) => {
                let d = childdbValue.val();
                Object.keys(d).forEach(function(key) {
                    getList.push({"name":childdbValue.ref.getKey(),"password":key,"score":d[key]});
                   })
...
function submitName(){
    ...
    passw = document.getElementById('passWord').value;
        ...
        if(str===""){
            ...
            alert('Password must less than 12 letters');
        }else{
            nameFlag = nameFlag+1;
            document.getElementById('userName').style.display='none';
            document.getElementById('passWord').style.display='none';
            document.getElementById('signin').innerHTML= "Welcome : " + str;
            document.getElementById('in').style.display="none";
                    document.getElementById('re').style.display="none";
            for(let x = 0;x<getList.length;x++){
                if(getList[x].name.toString() ==str.toString()){
                    if(getList[x].password != passw){
                        alert("password not correct or the name is already exist");
                        window.location.reload();
                   
                    }else{
                    document.getElementById('in').style.display="none";
                    document.getElementById('re').style.display="none";
                    document.getElementById('oldscore').innerHTML="Your total score: "+getList[x].score;
                    oscore = getList[x].score;
                    winscore = oscore;
                    document.getElementById('userScore').innerHTML = oscore;}
                }
                // if(getList[x].password != passw){
                //     alert("password not correct or the name is already exist");
                //     window.location.reload();
...
function getTheList() {
    ...
    for(let x = 0;x<5;x++){
        if(tmp[x]==null){
            document.getElementById("no"+x).innerHTML = "No record";
        }else{
            document.getElementById("no"+x).innerHTML = tmp[x].name +" : "+ tmp[x].score;
        }
...
function saveUserScore(){
    //let uScore;
    let uName; 
    document.getElementById('oldscore').innerHTML = "Your total score: " +  winscore;
    ...
    db.child(uName).child(passw).set(winscore);
